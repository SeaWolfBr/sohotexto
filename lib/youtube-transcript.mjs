...
const YOUTUBE_INNERTUBE_ANDROID_KEY =
  process.env.YOUTUBE_INNERTUBE_ANDROID_KEY?.trim() || null;
...
async function fetchPlayerResponseFromInnertube(videoId, fetchImpl) {
  if (!YOUTUBE_INNERTUBE_ANDROID_KEY) {
    throw new Error("Fallback Innertube indisponivel: chave nao configurada no servidor.");
  }

  const response = await fetchImpl(
    `https://www.youtube.com/youtubei/v1/player?key=${YOUTUBE_INNERTUBE_ANDROID_KEY}`,
    {
      method: "POST",
      headers: {
...
export async function fetchTranscriptFromInnertube({
  videoId,
  lang = null,
  fetchImpl = fetch,
}) {
  const playerResponse = await fetchPlayerResponseFromInnertube(videoId, fetchImpl);
  const { track, baseResult } = buildResultFromPlayerResponse(playerResponse, {
    videoId,
    lang,
  });
...
export async function fetchYouTubeTranscript(
  ...
  try {
    return await fetchTranscriptFromInnertube({ videoId, lang, fetchImpl });
  } catch (innertubeError) {
    errors.push(innertubeError);
  }
  ...
  throw new Error(
    "Nao foi possivel obter a transcricao deste video. O YouTube nao expôs uma trilha de legenda acessivel para esta requisicao.",
  );
}
