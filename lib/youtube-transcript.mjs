...
async function fetchPlayerResponseFromInnerTube(videoId, fetchImpl) {
  ...
  if (!response.ok) {
    throw new Error(`InnerTube player API falhou (${response.status}).`);
  }
...
export async function fetchYouTubeTranscript(
  ...
  try {
    ...
    const { track, baseResult } = buildResultFromPlayerResponse(playerResponse, { videoId, lang });
    const transcriptPayload = await fetchTranscriptTrack(track, fetchImpl);
    const transcript = parseTranscriptEvents(transcriptPayload);
    return finalizeResult({ ...baseResult, transcript });
