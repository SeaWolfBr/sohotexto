...
async function runYtDlpSubtitleDownload(input, lang) {
  if (!YTDLP_ENABLED) {
  ...
  const tmpDir = path.join(os.tmpdir(), `justext-ytdlp-${crypto.randomUUID()}`);
  await fs.mkdir(tmpDir, { recursive: true });

  const subLangs = lang ? `${lang},pt,pt-orig,en` : "pt,pt-orig,en,all";
  const cookieFile = YTDLP_COOKIE_FILE || temporaryCookieFile;
  ...
  args.push("-o", path.join(tmpDir, "%(id)s"), "--", input);
  ...
  try {
    for (const candidate of getYtDlpCandidates()) {
      try {
        ...
        if (!subFile) {
          throw new Error("yt-dlp nao gerou arquivo de legenda.");
        }
        ...
        const detectedLang = langMatch ? langMatch[1] : lang || "pt";

        return { payload: parsed, languageCode: detectedLang };
      } catch (error) {
...
async function fetchTranscriptFromYtDlp(input, lang) {
  const { payload, languageCode } = await runYtDlpSubtitleDownload(input, lang);
  const transcript = parseTranscriptEvents(payload);
